...
static size_t ReadILOperand(LowLevelILFunction& il, const Instruction& instr, size_t i,
    size_t addrSize = 4, bool isAddress = false)
{
    ...
    Architecture* arch, uint64_t addr, LowLevelILFunction& il, Instruction& instr, size_t addrSize)
{
	...
	case MIPS_ADDIU:
		if (op2.reg == REG_ZERO)
			...
			il.AddInstruction(SetRegisterOrNop(
			    il, 4, op1.reg, il.Add(4, ReadILOperand(il, instr, 2), ReadILOperand(il, instr, 3))));
		break;
	case MIPS_SUB:
	case MIPS_SUBU:
		il.AddInstruction(SetRegisterOrNop(
		    il, 4, op1.reg, il.Sub(4, ReadILOperand(il, instr, 2), ReadILOperand(il, instr, 3))));
		break;
	case MIPS_AND:
		il.AddInstruction(SetRegisterOrNop(
		    il, 4, op1.reg, il.And(4, ReadILOperand(il, instr, 2), ReadILOperand(il, instr, 3))));
		break;
	case MIPS_ANDI:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.And(4, ReadILOperand(il, instr, 2),
		        il.Operand(1, il.Const(4, 0x0000ffff & op3.immediate)))));
		break;
	case MIPS_DIV:
		il.AddInstruction(il.SetRegister(
		    4, REG_LO, il.DivSigned(4, ReadILOperand(il, instr, 1), ReadILOperand(il, instr, 2))));
		il.AddInstruction(il.SetRegister(
		    4, REG_HI, il.ModSigned(4, ReadILOperand(il, instr, 1), ReadILOperand(il, instr, 2))));
		break;
	case MIPS_DIVU:
		il.AddInstruction(il.SetRegister(
		    4, REG_LO, il.DivUnsigned(4, ReadILOperand(il, instr, 1), ReadILOperand(il, instr, 2))));
		il.AddInstruction(il.SetRegister(
		    4, REG_HI, il.ModUnsigned(4, ReadILOperand(il, instr, 1), ReadILOperand(il, instr, 2))));
		break;
	case MIPS_MUL:
		il.AddInstruction(SetRegisterOrNop(
		    il, 4, op1.reg, il.Mult(4, ReadILOperand(il, instr, 2), ReadILOperand(il, instr, 3))));
		break;
	case MIPS_XOR:
		il.AddInstruction(SetRegisterOrNop(
		    il, 4, op1.reg, il.Xor(4, ReadILOperand(il, instr, 2), ReadILOperand(il, instr, 3))));
		break;
	case MIPS_XORI:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.Xor(4, ReadILOperand(il, instr, 2),
		        il.Operand(1, il.Const(4, 0x0000ffff & op3.immediate)))));
		break;
	case MIPS_B:
	case MIPS_J:
	...
	case MIPS_BNEL:
		ConditionalJump(
		    arch, il, GetConditionForInstruction(il, instr), addrSize, op3.immediate, addr + 8);
		return false;
	...
	case MIPS_BLTZL:
		ConditionalJump(
		    arch, il, GetConditionForInstruction(il, instr), addrSize, op2.immediate, addr + 8);
		return false;
	...
	case MIPS_CLO:
		...
		il.AddInstruction(il.If(
		    il.CompareNotEqual(4,
		        il.And(4, il.ShiftLeft(4, ReadILOperand(il, instr, 2), il.Register(1, LLIL_TEMP(0))),
	...
	case MIPS_LBU:
		il.AddInstruction(
		    SetRegisterOrNop(il, 4, op1.reg, il.ZeroExtend(4, ReadILOperand(il, instr, 2, 1))));
		break;
	case MIPS_LB:
		il.AddInstruction(
		    SetRegisterOrNop(il, 4, op1.reg, il.SignExtend(4, ReadILOperand(il, instr, 2, 1))));
		break;
	case MIPS_MFHI:
	...
	case MIPS_MOVN:
		il.AddInstruction(il.If(
		    il.CompareNotEqual(4, ReadILOperand(il, instr, 3), il.Const(4, 0)), trueCode, falseCode));
		il.MarkLabel(trueCode);
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg, ReadILOperand(il, instr, 2)));
		il.MarkLabel(falseCode);
		break;
	case MIPS_MOVZ:
		il.AddInstruction(il.If(
		    il.CompareEqual(4, ReadILOperand(il, instr, 3), il.Const(4, 0)), trueCode, falseCode));
		il.MarkLabel(trueCode);
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg, ReadILOperand(il, instr, 2)));
		il.MarkLabel(falseCode);
		break;
		...
		il.AddInstruction(il.SetRegisterSplit(4, REG_HI, REG_LO,
		    il.Sub(8, il.Register(8, LLIL_TEMP(0)),
		        il.MultDoublePrecUnsigned(
		            8, ReadILOperand(il, instr, 1), ReadILOperand(il, instr, 2)))));
		break;
	case MIPS_MULT:
		il.AddInstruction(il.SetRegisterSplit(4, REG_HI, REG_LO,
		    il.MultDoublePrecSigned(8, ReadILOperand(il, instr, 1), ReadILOperand(il, instr, 2))));
		break;
	case MIPS_MULTU:
		il.AddInstruction(il.SetRegisterSplit(4, REG_HI, REG_LO,
	...
	case MIPS_NEG:
	case MIPS_NEGU:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg, il.Neg(4, ReadILOperand(il, instr, 2))));
		break;
	case MIPS_NOT:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg, il.Not(4, ReadILOperand(il, instr, 2))));
		break;
	case MIPS_NOR:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
	...
	case MIPS_OR:
		il.AddInstruction(SetRegisterOrNop(
		    il, 4, op1.reg, il.Or(4, ReadILOperand(il, instr, 2), ReadILOperand(il, instr, 3))));
		break;
	case MIPS_ORI:
		if (op2.reg == REG_ZERO)
			il.AddInstruction(
			...
			il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
			    il.Or(4, ReadILOperand(il, instr, 2),
			        il.Operand(1, il.Const(4, 0x0000ffff & op3.immediate)))));
		break;
	case MIPS_RDHWR:
	...
	case MIPS_SW:
		il.AddInstruction(
		    il.Store(4, GetILOperandMemoryAddress(il, op2, addrSize), ReadILOperand(il, instr, 1)));
		break;
	case MIPS_SD:
		il.AddInstruction(
		    il.Store(8, GetILOperandMemoryAddress(il, op2, addrSize), ReadILOperand(il, instr, 1)));
		break;
	case MIPS_SWC1:
	...
	case MIPS_EXT:
		...
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.And(4, il.Const(4, (1 << op4.immediate) - 1),
		        il.ShiftLeft(4, ReadILOperand(il, instr, 2), il.Const(1, op3.immediate)))));
		break;
	case MIPS_INS:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.Or(4,
		        ...
		        il.And(4, il.Const(4, (1 << op4.immediate) - 1), ReadILOperand(il, instr, 2)))));
		break;
	case MIPS_LUI:
	...
	case MIPS_SRAV:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.ArithShiftRight(4, ReadILOperand(il, instr, 2), ReadILOperand(il, instr, 3))));
		break;
	case MIPS_SLT:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.BoolToInt(4, il.CompareSignedLessThan(
		                        4, ReadILOperand(il, instr, 2), ReadILOperand(il, instr, 3)))));
		break;
	case MIPS_SLTI:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.BoolToInt(4,
	...
	case MIPS_SLTIU:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.BoolToInt(4, il.CompareUnsignedLessThan(
		                        4, ReadILOperand(il, instr, 2), il.Const(4, op3.immediate)))));
		break;
	case MIPS_SLTU:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.BoolToInt(4, il.CompareUnsignedLessThan(
	...
	case MIPS_SLLV:
		il.AddInstruction(SetRegisterOrNop(
		    il, 4, op1.reg, il.ShiftLeft(4, ReadILOperand(il, instr, 2), ReadILOperand(il, instr, 3))));
		break;
	case MIPS_SB:
		il.AddInstruction(il.Store(1, GetILOperandMemoryAddress(il, op2, addrSize),
		    il.LowPart(1, ReadILOperand(il, instr, 1))));
		break;
	case MIPS_TRAP:
	...
	case MIPS_TEQ:
		ConditionExecute(il,
		    il.CompareEqual(4, ReadILOperand(il, instr, 1), ReadILOperand(il, instr, 2)), il.Trap(0));
		break;
	case MIPS_TNE:
	case MIPS_TNEI:
		ConditionExecute(il,
		    il.CompareNotEqual(4, ReadILOperand(il, instr, 1), ReadILOperand(il, instr, 2)),
		    il.Trap(0));
		break;
	case MIPS_TGE:
	case MIPS_TGEI:
		ConditionExecute(il,
		    il.CompareSignedGreaterEqual(4, ReadILOperand(il, instr, 1), ReadILOperand(il, instr, 2)),
		    il.Trap(0));
		break;
	case MIPS_TGEIU:
	case MIPS_TGEU:
		ConditionExecute(il,
		    il.CompareUnsignedGreaterEqual(4, ReadILOperand(il, instr, 1), ReadILOperand(il, instr, 2)),
		    il.Trap(0));
		break;
	case MIPS_TLT:
	case MIPS_TLTI:
		ConditionExecute(il,
		    il.CompareSignedLessThan(4, ReadILOperand(il, instr, 1), ReadILOperand(il, instr, 2)),
		    il.Trap(0));
		break;
	case MIPS_TLTIU:
	case MIPS_TLTU:
		ConditionExecute(il,
		    il.CompareUnsignedLessThan(4, ReadILOperand(il, instr, 1), ReadILOperand(il, instr, 2)),
		    il.Trap(0));
		break;
	case MIPS_LH:
	case MIPS_LHI:
		il.AddInstruction(
		    SetRegisterOrNop(il, 4, op1.reg, il.SignExtend(4, ReadILOperand(il, instr, 2, 2))));
		break;
	case MIPS_LHU:
		il.AddInstruction(
		    SetRegisterOrNop(il, 4, op1.reg, il.ZeroExtend(4, ReadILOperand(il, instr, 2, 2))));
		break;
	case MIPS_LWL:
	case MIPS_LWR:
	...
	case MIPS_SEB:
		il.AddInstruction(SetRegisterOrNop(
		    il, 4, op1.reg, il.SignExtend(4, il.LowPart(1, ReadILOperand(il, instr, 2)))));
		break;
	case MIPS_SEH:
		il.AddInstruction(SetRegisterOrNop(
		    il, 4, op1.reg, il.SignExtend(4, il.LowPart(2, ReadILOperand(il, instr, 2)))));
		break;
	case MIPS_SH:
		il.AddInstruction(il.Store(2, GetILOperandMemoryAddress(il, op2, addrSize),
		    il.LowPart(2, ReadILOperand(il, instr, 1))));
		break;
	case MIPS_SRL:
	case MIPS_SRLV:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.LogicalShiftRight(4, ReadILOperand(il, instr, 2), ReadILOperand(il, instr, 3))));
		break;
	case MIPS_SSNOP:
	case MIPS_NOP:
	...
	case MIPS_CVT_D_W:
		il.AddInstruction(il.SetRegisterSplit(
		    4, op1.reg | 1, op1.reg & (~1), il.IntToFloat(8, ReadILOperand(il, instr, 2))));
		break;
	case MIPS_CVT_W_S:
